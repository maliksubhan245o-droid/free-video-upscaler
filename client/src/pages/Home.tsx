...
export default function Home() {
  ...
  const upscaleImage = async () => {
    ...
    setImageStatus("LOADING LOCAL AI MODEL");
    ...
    try {
      ...
      toast.success("AI image enhancement complete. Your image stayed in this browser.");
    } catch {
      const rendered = localEnhanceImage(image, image.naturalWidth, image.naturalHeight, canvas, 2);
      canvas.toBlob((blob) => { if (blob) setImageOutputUrl(URL.createObjectURL(blob)); }, "image/png");
      setImageStatus(`LOCAL FALLBACK · ${rendered.width}×${rendered.height}`);
      toast("AI model could not run on this device; a local enhanced output was created.");
    }
  };
